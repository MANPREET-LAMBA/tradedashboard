...
const Dashboard = () => {
  ...
  return (
    // CHANGE 1: min-h-screen for mobile, h-screen for desktop
    <div className="flex w-full min-h-screen lg:h-screen bg-[#0f172a] text-slate-200 font-sans relative overflow-x-hidden">
      ...
      {/* MAIN CONTENT */}
      {/* CHANGE 2: Removed overflow-hidden on mobile to allow trade list to push page down */}
      <main className="flex-1 flex flex-col min-w-0 p-4 lg:p-8 lg:overflow-hidden relative">
        
        {/* RIGHT SIDE ANIMATION PANEL */}
        {/* <div className="hidden xl:flex absolute right-8 top-32 flex-col gap-6 w-48 z-10 animate-in fade-in slide-in-from-right duration-1000">
          <div className="bg-[#1e293b]/80 backdrop-blur-xl p-4 rounded-2xl border border-emerald-500/20 shadow-[0_0_20px_rgba(16,185,129,0.1)]">
            ...
            <div className="flex items-end gap-1 h-12 mb-3">
              {[1, 2, 3, 4, 5, 6].map((i) => (
                <div key={i} className="w-1.5 bg-emerald-500/40 rounded-full animate-bounce" style={{ animationDuration: `${0.5 + i * 0.1}s`, height: `${40 + Math.random() * 60}%` }} />
              ))}
            </div>
            <p className="text-[9px] text-slate-500 leading-tight">Webhook signals in <span className="text-slate-300">real-time</span>.</p>
          </div>
        </div> */}
        ...
        <header className="flex justify-between items-center mb-6 flex-shrink-0">
          ...
          <div className="flex items-center gap-4">
            <div className="hidden md:flex bg-slate-800/50 rounded-full px-4 py-1.5 items-center gap-2 border border-slate-700">
              ...
              <span className="text-[10px] uppercase tracking-wider font-bold text-slate-400">Live Active</span>
            </div>
            <div className="w-10 h-10 bg-slate-800 rounded-full border border-slate-700 flex items-center justify-center text-emerald-400"><User size={20} /></div>
          </div>
        </header>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 mb-6 bg-[#1e293b]/50 p-4 rounded-2xl border border-slate-700 backdrop-blur-md flex-shrink-0">
          ...
          <div className="sm:col-span-2 space-y-1">
            ...
            <div className="flex items-center gap-2">
              <input type="date" disabled={showAllTime} value={startDate} onChange={(e) => setStartDate(e.target.value)} style={{ colorScheme: "dark" }} className="flex-1 bg-slate-900 border border-slate-700 rounded-xl px-3 py-2 text-xs text-white" />
              <span className="text-slate-600 font-bold text-xs">to</span>
              <input type="date" disabled={showAllTime} value={endDate} onChange={(e) => setEndDate(e.target.value)} style={{ colorScheme: "dark" }} className="flex-1 bg-slate-900 border border-slate-700 rounded-xl px-3 py-2 text-xs text-white" />
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6 flex-shrink-0">
          <StatCard title="Net Profit" value={`${activeCurrency}${totalPnL.toLocaleString(undefined, { minimumFractionDigits: 2 })}`} icon={<TrendingUp className="text-emerald-400" />} trend="+12.5%" />
        ...
        <div className="mb-6 p-4 bg-emerald-500/5 border border-emerald-500/20 rounded-2xl flex-shrink-0">
          <div className="flex items-center gap-2 mb-1">
            <Info size={14} className="text-emerald-400" />
            <span className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-400">STRATEGY NOTE</span>
          </div>
          <p className="text-sm text-slate-400 italic leading-relaxed">{strategyNote}</p>
        </div>

        {/* DATA TABLE AREA */}
        {/* CHANGE 3: Removed h-screen/overflow logic on mobile to make trade list visible by scrolling down */}
                ...
                {filteredTrades.map((trade) => {
                  ...
                  return (
                    <tr key={trade._id} className="hover:bg-emerald-500/[0.03] transition-colors group">
                      ...
                      <td className="px-6 py-4"><span className={`inline-block px-3 py-1 rounded text-[10px] font-black ${trade.type === "buy" || trade.type === "sell" ? "bg-emerald-500/10 text-emerald-400" : "bg-slate-700 text-slate-300"}`}>{(trade.type || trade.signal || "N/A").toUpperCase()}</span></td>
...
const StatCard = ({ title, value, icon, trend }) => (
  <div className="bg-[#1e293b] p-4 lg:p-6 rounded-2xl border border-slate-700 hover:border-slate-500 transition-all shadow-lg group">
    <div className="flex justify-between items-start mb-4">
      ...
      {trend && <span className="text-[10px] text-emerald-400 bg-emerald-500/10 px-2.5 py-1 rounded-full font-black uppercase tracking-tighter">{trend}</span>}
    </div>
